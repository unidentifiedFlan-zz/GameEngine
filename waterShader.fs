...
float DirectionalShadowCalculation(int lightIndex, vec3 norm)
{
    ...
    float bias = max(0.05 * (1.0 - dot(norm, light.direction)), 0.0005);
...
vec3 CalcDirLight(int lightIndex, vec3 norm, vec3 viewDir)
{
	...
	vec3 ambient = light.ambient * material.ambient * texture(textureMap.diffuse, texCoord).rgb;

	float diff = max(dot(norm, lightDir), 0.0);
	vec3 diffuse = diff*light.diffuse* material.diffuse * texture(textureMap.diffuse, texCoord).rgb;
	
	vec3 reflectDir = reflect(-lightDir, norm);
	float spec = pow(max(dot(viewDir, reflectDir), 0.0), material.shininess);
	vec3 specular = material.specular * spec * light.specular;// * texture(textureMap.specular, texCoord).rgb;
...
vec3 CalcSpotLight(SpotLight light, vec3 norm, vec3 fragPos, vec3 viewDir )
{
    ...
    // combine results
    vec3 ambient = light.ambient * vec3(texture(textureMap.diffuse, texCoord));
    vec3 diffuse = light.diffuse * diff * vec3(texture(textureMap.diffuse, texCoord));
    vec3 specular = light.specular * spec * vec3(texture(textureMap.specular, texCoord));
...
//	}
   ...
   for(int i = 0; i < NUM_WAVES; i++) {
   	   norm.x -= waves[i].xAmplitude*cos(waves[i].direction.x * texCoord.x + waves[i].phase*time);
   	   norm.y -= waves[i].yAmplitude*cos(waves[i].direction.y * texCoord.y + waves[i].phase*time);
	   norm.z = 1.0;
	   norm = normalize(norm * 2.0 - 1.0); 
	   norm = normalize(TBN * norm); 
   }
